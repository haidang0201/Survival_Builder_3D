namespace TopsonGames
{
    ...
    [CreateAssetMenu(fileName = "MusketeerCombatSO", menuName = "TopsonGames/Combat Behaviours/Musketeer Combat")]
    public class MusketeerCombatSO : CombatBehaviourSO
    {
        ...
        [Header("Line Infantry Tactics")]
        [Tooltip("How many rows in the formation are allowed to fire simultaneously? (e.g., 2 for front and second rank)")]
        public int maxFiringRows = 2;
        [Tooltip("If true, the very first row will kneel while shooting. Subsequent rows will stand.")]
        public bool firstRowKneels = true;
        ...
        public override void TickCombat(Unit unit, float deltaTime, Coroutine movementRoutine)
        {
            ...
            {
                unit.ResetTarget();
                unit.animatorLink.SetRowInFormation(unit.rowInFormation);
        ...
        public override void OnUpdateFormation(Formation formation)
        {
            ...
            if (formation.archerDetectionTimer < 0)
            {
                ...
                if (formation.CurrentState == FormationState.Engaged || formation.currentArrows <= 0)
                {
                    ...
                    // 3. Status 'Shooting' sauber beenden (hat hier gefehlt!)
        ...
        {
            var eUnits = formation.GetUnits();
            if (eUnits == null || eUnits.Count == 0) return null;

            // Hole alle lebendigen Feinde dieser Formation
            var validEnemies = eUnits.Where(u => u != null && u.GetCurrentHealth() > 0).ToList();
            if (validEnemies.Count == 0) return null;

            // Anstatt das absolut nächste Ziel zu berechnen (was oft derselbe Feind ist), 
            // wählt jede Einheit nun ein zufälliges Ziel in der Feindformation aus.
            // Dadurch wird das Musketen-Feuer sauber in die feindliche Menge verteilt.
            return validEnemies[Random.Range(0, validEnemies.Count)];
        }

        public void MusketeerAttack(Unit unit)
        {
            if (!unit || unit.GetCurrentTarget() == null) return;

            if (unit.rowInFormation > maxFiringRows) return;
            ...
            if (unit.currentState == UnitState.Idle && IsFacingTarget(unit, unit.GetCurrentTarget().transform, 60))
            {
                ...
                if (unit.RangedWeapon != null && unit.RangedWeapon.animator != null)
                {
