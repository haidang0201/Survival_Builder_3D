namespace TopsonGames
{
    ...
    [CreateAssetMenu(fileName = "ArcherVisualizerSO", menuName = "TopsonGames/Visualizer/ArcherVisualizerSO")]
    public class ArcherVisualizerSO : VisualizerSO
    {
        ...
        public override void TickVisualize(CombatBehaviourSO combatBehaviourSO, Formation formation, LineRenderer lineRenderer)
        {
            ...
            var ArcherBehaviour = (combatBehaviourSO as ArcherCombatSO);

            Vector3 center = formation.CalculateUnitCenter();
            Vector3 forward = formation.GetUnits()[0].transform.forward.normalized;

            var archerSO = formation.UnitData.combatBehaviour as ArcherCombatSO;
            if (archerSO == null)
                return;

            float frontOffset = archerSO.archerFrontOffset;
            float detectionRange = archerSO.archerDetectionRange;
            float innerAngle = archerSO.archerInnerAngle;
            float outerAngle = archerSO.archerOuterAngle;
