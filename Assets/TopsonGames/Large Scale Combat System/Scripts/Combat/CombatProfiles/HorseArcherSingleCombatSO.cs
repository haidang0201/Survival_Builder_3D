namespace TopsonGames
{
    ...
    [CreateAssetMenu(fileName = "HorseArcherSingleCombatSO", menuName = "TopsonGames/Combat Behaviours/Horse Archer (Single Mesh)")]
    public class HorseArcherSingleCombatSO : CombatBehaviourSO
    {
        ...
        public override void TickCombat(Unit unit, float deltaTime, Coroutine movementRoutine)
        {
            ...
            if (target == null || target.currentHealth <= 0)
            {
                ...
                if (unit.animatorLink is AnimatorLinkMeshHorseArchersSingle link) link.StopAiming();
            ...
            else if (distanceToTarget <= archerDetectionRange)
            {
                ...
                if (unit.animatorLink is AnimatorLinkMeshHorseArchersSingle haLink)
                {
        ...
        private void FireDirectionalArrow(Unit unit, Formation targetFormation)
        {
            ...
            if (closestEnemy != null)
            {
                ...
                if (unit.animatorLink is AnimatorLinkMeshHorseArchersSingle horseArcherLink)
                {
        ...
        public override void TickIdle(Unit unit, NavMeshAgent agent, Formation formation, Coroutine movementRoutine)
        {
            if (agent.enabled && agent.isOnNavMesh)
            {
                if (!agent.pathPending && agent.remainingDistance <= agent.stoppingDistance)
                {
                    ...
                    {
                        unit.StopMovement();
                        if (unit.GetCurrentTarget() == null && unit.animatorLink is AnimatorLinkMeshHorseArchersSingle link)
                        {
                            link.StopAiming();
                        }
                    }
